...
const BookingTypePage = () => {
  const [form, setForm] = useState({ BookingTypeName: '', BookingTypeDetail: '' });
  const [editingId, setEditingId] = useState(null);
  const [bookingTypes, setBookingTypes] = useState([]);
  const [action, setAction] = useState('get');
  const [showForm, setShowForm] = useState(false);
  const [expandedRows, setExpandedRows] = useState([]);
  const [originBookingTypes, setOriginBookingTypes] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const bookingTypesPerPage = 10;
  const navigate = useNavigate();

  // Calculate total pages
  const totalPages = Math.ceil(bookingTypes.length / bookingTypesPerPage);

  // Get booking types for the current page
  const currentBookingTypes = bookingTypes.slice((currentPage - 1) * bookingTypesPerPage, currentPage * bookingTypesPerPage);
...
useEffect(() => {
  ...
  const fetchBookingTypes = async () => {
    ...
    setOriginBookingTypes(response.data);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (action === 'search') {
        const filteredBookingTypes = originBookingTypes.filter((originBookingType) => {
          let isMatch = true;
          if (form.BookingTypeName) {
            isMatch = isMatch && originBookingType.BookingTypeName === form.BookingTypeName;
          }
          if (form.BookingTypeDetail) {
            isMatch = isMatch && originBookingType.BookingTypeDetail === form.BookingTypeDetail;
          }
          return isMatch;
        });
      ...
      const existingBookingType = bookingTypes.find(bt => bt.BookingTypeName === form.BookingTypeName);
      if (existingBookingType) {
        if (editingId) {
          if (existingBookingType._id !== editingId) {            
            return;
          }
        } else {          
          return;
      ...
      if (editingId) {
        setAction('update');
        await axios.put(`http://103.124.92.222:6868/rooms/booking-type/${editingId}`, form);
      } else {
        setAction('create');
        await axios.post('http://103.124.92.222:6868/rooms/booking-type', form);
      }
      setForm({ BookingTypeName: '', BookingTypeDetail: '' });
      setAction('get');
      setEditingId(null);
      setShowForm(false);
      fetchBookingTypes();
  ...
  const handleDelete = async (id) => {
    await axios.delete(`http://103.124.92.222:6868/rooms/booking-type/${id}`);
    fetchBookingTypes();
  };  
  ...
  return (
    ...
    <Container>
      ...
      <div className="mb-3">
        ...
        {/* <Button onClick={handleSearchNew} className="me-2">Search New</Button>
        {showForm && action === 'search' && (
          <Button onClick={handleClearSearch} className="me-2">Clear Search</Button>
        )} */}
        {showForm && <Button onClick={handleHideForm} variant="link" className="transparent-button me-2">Ẩn Form</Button>}
        {showForm && (
            ...
            <Table striped bordered hover responsive>
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <FormControl
                      name="BookingTypeDetail"
                      value={form.BookingTypeDetail}
                      onChange={handleChange}
                  ...
                  <td>
                    <Button type="submit" variant="link" className="transparent-button me-2">
                      {action === 'search' ? 'Tìm' : editingId ? 'Sửa' : 'Tạo'}
                    </Button>
                  </td>
                </tr>
              </tbody>
      ...
      <Table striped bordered hover responsive>
        ...
        <tbody>
          {currentBookingTypes.map((bookingType, index) => (
            <tr key={bookingType._id}>
              <td>{index + 1}</td>
              <td>{bookingType.BookingTypeName}</td>
              <td>{bookingType.BookingTypeDetail}</td>
              <td>
                <Button variant="link" className="transparent-button" onClick={() => handleEdit(bookingType)}>Sửa</Button>
                <Button variant="link" className="transparent-button" onClick={() => handleDelete(bookingType._id)}>Xóa</Button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <Pagination>
        {[...Array(totalPages).keys()].map((page) => (
          <Pagination.Item key={page + 1} active={page + 1 === currentPage} onClick={() => handlePageChange(null, page + 1)}>
            {page + 1}
          </Pagination.Item>
        ))}
      </Pagination>
